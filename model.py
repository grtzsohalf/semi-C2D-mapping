...
class Model(nn.Module):
    ...
    def forward(self, batch_size, aud_feats, aud_lengths, aud_orders, txt_feats, txt_lengths, txt_orders, txt_labels):
        ...
        in_aud_reconstruction_loss = self.compute_reconstruction_loss(aud_reconstructed_target, target_aud_feats, True, target_aud_lengths) 
        cr_aud_reconstruction_loss = self.compute_reconstruction_loss(aud_reconstructed_paired, paired_aud_feats, True, paired_aud_lengths)
        in_txt_CE_loss = self.compute_CE_loss(txt_reconstructed_target, target_txt_labels, True, target_txt_lengths) 
        cr_txt_CE_loss = self.compute_CE_loss(txt_reconstructed_paired, paired_txt_labels, True, paired_txt_lengths)
        if self.if_x:
            ...
            x_loss = torch.tensor(0., device=device)
        ...
        return target_phn_hiddens, target_spk_hiddens, target_txt_hiddens, \
            in_aud_reconstruction_loss, cr_aud_reconstruction_loss, in_txt_CE_loss, cr_txt_CE_loss, x_loss, pos_paired_loss, neg_paired_loss
